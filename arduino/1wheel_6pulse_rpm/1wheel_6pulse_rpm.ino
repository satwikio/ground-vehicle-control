...
const int numMotor = 1;
const int speedPinArray[numMotor] = {22};
const unsigned long refreshTime = 300000000;
unsigned long prevTimeArray[numMotor] = {0};
unsigned long prevPulseWidth[numMotor] = {0};
unsigned long currPulseWidth[numMotor] = {0};
int prevSignalArray[numMotor] = {0};
...
//  {0,0,0,0,0,0}};
  ...
  for(int motor=0; motor<numMotor; motor++){
    if (currPulseWidth[motor] != 0){
      rpmArray[motor] = 1321860 / currPulseWidth[motor];
    }
    else{
      rpmArray[motor] = 0;
    }
