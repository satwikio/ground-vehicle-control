...
const int speedPinArray[numMotor] = {22,23};
const unsigned long refreshTime = 300000000;
unsigned long prevTimeArray[numMotor] = {0,0};
unsigned long prevPulseWidth[numMotor] = {0,0};
unsigned long currPulseWidth[numMotor] = {0,0};
int prevSignalArray[numMotor] = {0,0};
...
void loop() {
  ...
  for(int motor=0; motor<numMotor; motor++){
    if(currSignalArray[motor] != prevSignalArray[motor]){
      if (impulseTimeArray[motor][0] != 0 && impulseTimeArray[motor][1] != 0 && impulseTimeArray[motor][2] != 0){
...
//  {0,0,0,0,0,0}};
  ...
  for(int motor=0; motor<numMotor; motor++){
    if (currPulseWidth[motor] != 0){
      rpmArray[motor] = 697511.00 / currPulseWidth[motor];
    }
    else{
      rpmArray[motor] = 0;
    }
